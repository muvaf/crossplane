...
func TestPublisherChain(t *testing.T) {
		...
		"SuccessfulPublisher": {
			p: PublisherChain{
				ManagedConnectionPublisherFn{
					ApplyConnectionFn: func(_ context.Context, mg Managed, c ConnectionDetails) error {
						return nil
					},
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			got := tc.p.ApplyConnection(tc.args.ctx, tc.args.mg, tc.args.c)
...
func TestAPISecretPublisher(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			got := a.ApplyConnection(tc.args.ctx, tc.args.mg, tc.args.c)
			if diff := cmp.Diff(tc.want, got, test.EquateErrors()); diff != "" {
				t.Errorf("Publish(...): -want, +got:\n%s", diff)
			}
		})
