...
func UpdateParameters(p *v1alpha1.GlobalAddressParameters, observed *compute.Address) {
	p.Address = gcp.InitializeString(p.Address, observed.Address)
	p.AddressType = gcp.InitializeString(p.AddressType, observed.AddressType)
	p.IPVersion = gcp.InitializeString(p.IPVersion, observed.IpVersion)
	p.Network = gcp.InitializeString(p.Network, observed.Network)
	p.PrefixLength = gcp.InitializeInt64(p.PrefixLength, observed.PrefixLength)
	p.Purpose = gcp.InitializeString(p.Purpose, observed.Purpose)
	p.Subnetwork = gcp.InitializeString(p.Subnetwork, observed.Subnetwork)
}
